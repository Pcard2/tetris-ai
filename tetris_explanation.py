...
def printLine(line):
    for element in line:
        if element == 1: # I
            print("\033[1m\033[34m"+str(element)+" \033[0m", end='') # BLUE
        elif element == 2: # O
            print("\033[1m\033[33m"+str(element)+" \033[0m", end='') # YELLOW
        elif element == 3: # S
            print("\033[1m\033[32m"+str(element)+" \033[0m", end='') # GREEN
        elif element == 4: # Z
            print("\033[1m\033[31m"+str(element)+" \033[0m", end='') # RED
        elif element == 7: # T
            print("\033[1m\033[35m"+str(element)+" \033[0m", end='') # PURPLE
        else:
...
def printGrid(grid): # prints the grid line
    ...
    # print("\033[35mThis is blue font.\033[0m")
    # print("\033[36mThis is blue font.\033[0m")
    # print("\033[37mThis is blue font.\033[0m")
    ...
    for k in range(len(grid)):
        if not 0 in grid[k]:
            for element in grid[k]:
                print("\033[30m"+str(element)+" \033[0m", end='') # GRAY
                print(str(element)+" ", end='')
            sleep(2)
            for yReplace in range(k, 0, -1): # start:y end:0 step:-1
...
def newShape(i):
    shapeNum = [0,1,2,3,6]
    shape = tetrominos[shapeNum[i]]
    shapeChar = tetrominosStr[shapeNum[i]]
...
currentShape, xp, yp, shapeChar= newShape(0)
nextShape, xp, yp, nextShapeChar = newShape(1)
currentRotation = 0
changePiece = False
paused = False
gameOver = False
...
while executant:
    ...
    if not paused and not gameOver:
        ...
        if changePiece:
            ...
            nextShape, xp, yp, nextShapeChar = newShape(j)
            changePiece = False
    elif paused:
        drawPaused()
    elif gameOver:
